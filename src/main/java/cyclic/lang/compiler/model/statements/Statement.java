...
public abstract class Statement{
	...
	public static Statement fromAst(CyclicLangParser.StatementContext ctx, Scope in, List<String> imports){
		if(ctx.block() != null)
			return new BlockStatement(ctx.block().statement().stream().map(k -> fromAst(k, in, imports)).collect(Collectors.toList()), in);
		else if(ctx.varDecl() != null)
			return new VarStatement(in, ctx.varDecl().id().getText(), TypeResolver.resolve(ctx.varDecl().type().getText(), imports).orElseThrow(), ctx.varDecl().value() != null ? Value.fromAst(ctx.varDecl().value(), imports) : null, true);
		else if(ctx.varAssignment() != null)
			return new VarStatement(in, ctx.varAssignment().id().getText(), null, Value.fromAst(ctx.varAssignment().value(), imports), false);
